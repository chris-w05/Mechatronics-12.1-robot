...
void L298NMotorDriverMega::init()
{
  // Define pinMode for the pins. We no longer depend on timer hardware.
  pinMode(_M1PWM, OUTPUT);
  pinMode(_M1C, OUTPUT);
  pinMode(_M1D, OUTPUT);
  pinMode(_M2PWM, OUTPUT);
  ...
  // Ensure outputs are in a safe default state (coast)
  digitalWrite(_M1C, LOW);
  digitalWrite(_M1D, LOW);
...
// Helper: map 0..400 -> 0..255 (integer math)
static inline uint8_t map400to255(int speed400)
{
  // clamp
  if (speed400 < 0)
    speed400 = 0;
  if (speed400 > 400)
    speed400 = 400;
  // original code used speed * 51 / 80; that's equivalent to round(255 * speed / 400)
  return (uint8_t)((speed400 * 255) / 400);
}
...
void L298NMotorDriverMega::setM1Speed(int speed)
{
  ...
  if (speed > 400) // Max PWM dutycycle (logical scale)
    speed = 400;

  // direction pins
  if (forward ^ _flipM1) // if speed was negative XOR flip flag
  {
    digitalWrite(_M1C, HIGH);
    digitalWrite(_M1D, LOW);
  }
  ...
  uint8_t pwmVal = map400to255(speed);
  analogWrite(_M1PWM, pwmVal);
}
...
void L298NMotorDriverMega::setM1Brake(int brake)
{
  // To brake, set both direction lines LOW (short the outputs), and write PWM on enable
  digitalWrite(_M1C, LOW);
  digitalWrite(_M1D, LOW);
  ...
  uint8_t pwmVal = map400to255(brake);
  analogWrite(_M1PWM, pwmVal);
}
